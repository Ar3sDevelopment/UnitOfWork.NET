...
type Repository<'TEntity, 'TDTO when 'TEntity : not struct and 'TEntity : equality and 'TEntity : null and 'TDTO : equality and 'TDTO : null and 'TDTO : not struct>(manager) = 
    ...
    interface IRepository<'TEntity, 'TDTO> with
        ...
        member this.All(take, skip, sort, filter, whereFunc) = this.All(take, skip, sort, filter, whereFunc, this.DTOBuilder().BuildList)
    ...
    member this.DeleteAsync(entity : 'TEntity, [<ParamArray>] ids) = async { this.Delete(entity, ids) } |> Async.StartAsTask

...
type UnitOfWork internal (context : DbContext, autoContext) as uow = 
    ...
    let isRepository (t : Type) = 
        t.IsAssignableTo<IRepository>() && t.IsInterface |> not && t.IsAbstract |> not && t <> typeof<Repository> && t.GetType().GetGenericTypeDefinition() <> typedefof<Repository<_>> && t.GetType().GetGenericTypeDefinition() <> typedefof<Repository<_, _>> 
        && t.GetType().GetGenericTypeDefinition() <> typedefof<ListRepository<_, _, _>> && container.IsRegistered(t) |> not

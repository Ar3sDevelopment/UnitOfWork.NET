...
type UnitOfWork internal (context : DbContext) = 
    ...
    member this.Transaction(body : Action<IUnitOfWork>) = (this :> IUnitOfWork).Transaction(body)
    member this.TransactionSaveChanges(body : Action<IUnitOfWork>) = (this :> IUnitOfWork).TransactionSaveChanges(body)

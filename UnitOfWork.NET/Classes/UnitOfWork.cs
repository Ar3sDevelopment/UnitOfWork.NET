...
namespace UnitOfWork.NET.Classes
{
	public class UnitOfWork : IUnitOfWork
	{
		...
		public UnitOfWork()
		{
			...
			cb.RegisterGeneric(typeof(Repository<,>)).AsSelf().As(typeof(IRepository<,>));
			cb.RegisterGeneric(typeof(ListRepository<,,>)).AsSelf().As(typeof(IListRepository<,,>));
			_container = cb.Build();
		...
		private bool IsRepository(Type t)
		{
			try
			{
				return t.IsAssignableTo<IRepository>() && !t.IsInterface && !t.IsAbstract && t != typeof(Repository)
				&& ((t.IsGenericType && t.GetGenericTypeDefinition() != typeof(Repository<>)
				&& t.GetGenericTypeDefinition() != typeof(Repository<,>) && t.GetGenericTypeDefinition() != typeof(ListRepository<,,>)) || !t.IsGenericType)
		...
		{
			if (!repositoryType.IsInterface && !repositoryType.IsAbstract && !_container.IsRegistered(repositoryType))
			{
				var cb = new ContainerBuilder();
